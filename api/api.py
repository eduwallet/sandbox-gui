...
def switch():
  form = session.get('form')
  if form is None:
      logging.error("switch request failed")
      message = {
          "status": "error"
      }
      return json.dumps(message)
  ...
  create_url = config['switch']['url'] + "/create-offer"

  headers = {
      "Content-Type": "application/json",
      "Authorization": f"Bearer {config['switch']['token']}"
  }
...
def switch_status():
  ...
  check_url = config['switch']['url'] + "/check-offer"
  ...
  if status == 'CREDENTIAL_ISSUED' and revoke:
      ...
      revoke_url = config['switch']['url'] + "/revoke-credential"
